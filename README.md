...
**Download web novels and light novels, read them offline, and listen to them with text‑to‑speech — on Windows and Android.**

NovelGrabber is a two‑app project that turns web serials into a clean, ad‑free personal library. It ships with a built‑in browser that clears Cloudflare and login walls, a proper reader with themes and TTS, and EPUB/PDF export. There's no account, no server, and nothing leaves your device.

| | |
|---|---|
| 🖥️ **Desktop** | Windows app (C# / WinForms + WebView2) |
| 📱 **Android** | Native app (Kotlin, WebView UI) — uses the phone's **on‑device Google TTS voices** |
...
- **Text‑to‑speech**
  - **Android:** the phone's own **on‑device Google voices** (the whole reason the native app exists), with background playback + lock‑screen controls.
  - **Desktop:** free Microsoft **Edge neural voices** out of the box, plus optional **Google Cloud TTS** with your own key. Includes a "read any pasted text" mode.
...
Per‑site rules live in [`sites.json`](sites.json) (CSS selectors + navigation) and can be edited **without recompiling**.
...
**Requirements:** Android Studio / Android SDK (compileSdk 36, minSdk 24), JDK 21.

```bash
cd "Novel Scraper Final Android version"
./gradlew assembleDebug
# APK: app/build/outputs/apk/debug/app-debug.apk
```

Install the APK by sideloading it (enable "Install unknown apps" for your file manager). For TTS, install/enable **Speech Recognition & Synthesis by Google** so the on‑device voices are available.

---

## How it works

Both apps share the same approach and on‑disk format:
...
```
Novel Scraper Final/                 # Windows app (C#)
├── MainForm.cs, Theme.cs, ...        # UI
├── Extractor.cs, SiteRules.cs        # scraping
├── Library.cs, EpubWriter.cs, ...    # storage & export
├── reader.html                       # reader (WebView2)
└── sites.json                        # per-site selectors

Novel Scraper Final Android version/ # Android app (Kotlin)
└── app/src/main/
    ├── java/com/novelgrabber/app/    # MainActivity, Engine, TtsService, ...
    └── assets/                       # app.html, reader.html, sites.json
```
